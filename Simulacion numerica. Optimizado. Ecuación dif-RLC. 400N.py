# -*- coding: utf-8 -*-
"""
Created on Fri Jun 24 12:56:52 2022
...
for i in range(len(thetan)):
    integrando = lambda y: dT1_dt_fsup(y,0,thetan[i],omegan[i],d,l,m,K,R) - dT1_dt_finf(y,0,thetan[i],omegan[i],d,l,m,K,R)
...
for i in range(len(thetan)):
    integrando = lambda y: dT2_dt_fsup(y,0,thetan[i],omegan[i],d,l,m,K,R) - dT2_dt_finf(y,0,thetan[i],omegan[i],d,l,m,K,R)
    integral, integral_error = quad(integrando, -R, R)
...
for i in range(len(thetan)):
    integrando = lambda y: dT3_dt_fsup(y,0,thetan[i],omegan[i],d,l,m,K,R) - dT3_dt_finf(y,0,thetan[i],omegan[i],d,l,m,K,R)
    integral, integral_error = quad(integrando, -R, R)
...
def dSist(S,t):
    ...
    for n in range(len(thetan)): 
      Faprox = Faprox + an[n]*np.sin(n*(np.pi)*t/L)+ bn[n]*np.cos(n*(np.pi)*t/L)
    inductancia=0.002832
    resistencia=15.832
    return [I,(Faprox-resistencia*I)/inductancia]
...
for n in range(len(I)): 
    Faproxi = Faproxi + an[n]*np.sin(n*(np.pi)*T/L)+ bn[n]*np.cos(n*(np.pi)*T/L)
